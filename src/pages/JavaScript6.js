...
import AmEs6 from '../language/AmEs6';
import RuEs6 from '../language/RuEs6';
import FrEs6 from '../language/FrEs6';
import KoJavaScript6 from '../language/KoJavaScript6';
import EsJavaScript6 from '../language/EsJavaScript6';
import PtJavaScript6 from '../language/PtJavaScript6';
import JpJavaScript6 from '../language/JpJavaScript6';
  ...
  useEffect(() => {
    switch (language) {
      ...
      case 'am':
        ...
        break;
        case 'fr':
        document.title = "JavaScript ES6 (ECMAScript 2015) est la sixième version de JavaScript qui a introduit de nouvelles fonctionnalités et des améliorations de syntaxe dans le langage. Voici quelques fonctionnalités clés d'ES6 : | let et const : Ce sont de nouvelles déclarations de variables qui remplacent l'ancien mot-clé var. Elles fournissent de meilleures règles de portée et aident à éviter les erreurs courantes qui peuvent survenir avec var. | Fonctions fléchées | Modèles de chaînes | Opérateur de propagation | Destructuration | Classes | Modules | Promesses  ";
        break;
        case 'es':
        document.title = 'JavaScript ES6 (ECMAScript 2015) es la sexta versión de JavaScript que introdujo nuevas características y mejoras de sintaxis al lenguaje. Aquí hay algunas características clave de ES6: |  let y const: Estas son nuevas declaraciones de variables que reemplazan la antigua palabra clave var. Proporcionan mejores reglas de ámbito y ayudan a evitar errores comunes que pueden ocurrir con var. | Funciones de flecha | Literales de plantilla | Operador de propagación | Desestructuración | Clases |  Módulos | Promesas | El operador de propagación está representado por tres puntos (...) y se puede usar de varias maneras. Aquí hay algunos ejemplos: | Desestructuración en ES6';
        break;
        case 'pt':
        document.title = 'JavaScript ES6 (ECMAScript 2015) é a sexta versão do JavaScript que introduziu novos recursos e melhorias de sintaxe para a linguagem. Aqui estão algumas características-chave do ES6: | let e const: Estas são novas declarações de variáveis que substituem a antiga palavra-chave var. Elas fornecem melhores regras de escopo e ajudam a evitar erros comuns que podem ocorrer com var. | Funções de seta | Literais de modelo | Operador de propagação | Destruturação | Classes | Módulos | Promessas | O operador de propagação é representado por três pontos (...) e pode ser usado de várias maneiras. Aqui estão alguns exemplos ';
        break;
        case 'jp':
        document.title = 'JavaScript ES6（ECMAScript 2015）は、言語に新しい機能と構文の改善をもたらしたJavaScriptの第6版です。以下はES6の主な特長です： | let およびconst：これらは古いvarキーワードを置き換える新しい変数宣言です。より良いスコープルールを提供し、varによって発生する可能性のある一般的なミスを防ぐのに役立ちます。 | アロー関数 | テンプレートリテラル | スプレッド演算子 | 分割代入 | プロミス：プロミスはJavaScript | スプレッド演算子は3つのドット(...)で表され、さまざまな方法で使用できます。以下はいくつかの例です：';
        break;
        case 'ko':
        document.title = 'JavaScript ES6 (ECMAScript 2015)은 언어에 새로운 기능과 구문 개선을 도입한 JavaScript의 여섯 번째 버전입니다. 다음은 ES6의 주요 기능 중 일부입니다: |  let 그리고 const: 이들은 기존의 var 키워드를 대체하는 새로운 변수 선언입니다. 더 나은 스코프 규칙을 제공하고 var로 발생할 수 있는 일반적인 실수를 피하는 데 도움이 됩니다. |  화살표 함수 | 템플릿 리터럴 | 전개 연산자 | 구조 분해 | 클래스: ES6는 객체를 생성하고 그들의 동작을 정의하는 더 간단하고 직관적인 방법을 제공하는 새로운 클래스 구문을 도입했습니다. |  모듈 | 프로미스 | 전개 연산자는 세 개의 점 (...)으로 표시되며 다양한 방법으로 사용할 수 있습니다. 여기에 몇 가지 예시가 있습니다: ';
        break;
      default:
