...
import RuEs5_5 from '../language/RuEs5_5';
import AmEs5_5 from '../language/AmEs5_5';
import FrEs5_5 from '../language/FrEs5_5';
import ArJSEs5_5 from '../language/ArJSEs5_5';
import JpEs5_5 from '../language/JpJSEs5_5';
import EsJSEs5_5 from '../language/EsJSEs5_5';
 ...
 function JsEs5_5() {
  ...
  useEffect(() => {
    switch (language) {
      case 'en':
        document.title = 'JavaScript ES5-5 | Array | forEach and map | JavaScript Array Methods: push, pop, shift, unshift, splice, slice, join, concat, forEach, map | Apply Method | Self-Invoking Function ';
        break;
      case 'ru':
        document.title = 'JavaScript ES5-5 | Массив (Array) | forEach и map | Методы массивов JavaScript: push, pop, shift, unshift, splice, slice, join, concat, forEach, map | Метод apply | Самовызывающаяся функция ';
        break;
      case 'am':
        document.title = 'ՋավաՍկրիպտ ES5-5 | Զանգված (Array) | forEach և map | Զանգվածի մեթոդներ | Զանգվածների միավորում Մեթոդ apply | Ինքնականչվող ֆունկցիաները JavaScript-ում հզոր գործիք են մասնավոր շրջանակներ ստեղծելու, անունների բախումներից խուսափելու և գլոբալ փոփոխականները կառավարելու համար:';
        break;
        case 'fr':
        document.title = 'Tableau JavaScript - Méthodes, Exemples | forEach et map | Méthodes de tableau | Combinaison de tableaux | Méthode apply | Fonction auto-invocatrice';
        break;
        case 'es':
        document.title = 'En JavaScript, un array es una estructura de datos que te permite almacenar múltiples valores en una sola variable. Un array se declara usando corchetes, y sus elementos están separados por comas | Métodos de Arrays | Combinación de Arrays | forEach y map | Método apply ';
        break;
        case 'ar':
        document.title = 'مصفوفة, JavaScript, طرق المصفوفة, forEach, map, طريقة apply, الدالة التي تقوم بدعوة نفسها ';
        break;
        case 'jp':
        document.title = 'JavaScript、配列、メソッド、push、pop、shift、unshift、splice、slice、join、concat、forEach、map、apply、自己実行関数';
        break;
      default:
...
<p>In JavaScript, the apply() method is used to call a function with a given this value and arguments provided as an array (or an array-like object).</p>
...
<p>In this example, the function is defined and then immediately called using the opening and closing parentheses. This will output "Hello, World!" to the console.</p>

<p>Self-invoking functions are commonly used to create a private scope in which variables and functions can be defined without polluting the global namespace. Here's an example:</p>
